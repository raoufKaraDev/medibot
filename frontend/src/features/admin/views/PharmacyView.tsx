...
export function PharmacyView() {
  ...
  const [pharmForm, setPharmForm] = useState<{
    ...
    is_restricted_pediatric: boolean;
    is_high_risk: boolean;
    pediatric_mg_per_kg: string;
    maxdosemg24h: string;
    notes: string;
  }>({
    name: '',
  ...
  const fillPharmForm = (m: PharmaMed) =>
    setPharmForm({
      ...
      maxdosemg24h: m.maxdosemg24h != null ? String(m.maxdosemg24h) : '',
      notes: m.notes || '',
    });
  ...
  const submitPharm = async () => {
    ...
    const maxdoseRaw = pharmForm.maxdosemg24h.trim();
    const maxdoseParsed = maxdoseRaw === '' ? NaN : parseFloat(maxdoseRaw.replace(',', '.'));
    const maxdosemg24h = Number.isFinite(maxdoseParsed) ? maxdoseParsed : null;
          ...
          <Modal title={editing ? 'Modifier article' : 'Nouveau médicament'} onClose={() => setShowModal(false)} width="max-w-2xl">
            <div className="max-h-[80vh] overflow-y-auto space-y-6 pr-1">
              ...
              <div>
                ...
                <div className="mt-4">
                  <Field label="Dose max adulte / 24h (mg)">
                    <input
                      type="number"
                      min={0}
                      className={inp}
                      value={pharmForm.maxdosemg24h ?? ''}
                      onChange={(e) =>
                        setPharmForm((f) => ({
                          ...f,
                          maxdosemg24h: e.target.value,
                        }))
                      }
                      placeholder="Ex: 4000 pour Paracétamol"
                    />
                  </Field>
                </div>
                <div className="mt-3">
